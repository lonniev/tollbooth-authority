...
Dependency-tracking release: pins tollbooth-dpyc 0.88.0. Recorded after the fact —
the version shipped without a CHANGELOG entry at the time.

## 0.11.7 — 2026-08-22

### Changed — track tollbooth-dpyc 0.87.3

Dependency-tracking release: pins tollbooth-dpyc 0.87.3. Recorded after the fact —
the version shipped without a CHANGELOG entry at the time.
...
Dependency-tracking release: pins tollbooth-dpyc 0.87.2. Recorded after the fact —
the version shipped without a CHANGELOG entry at the time.
...
Dependency-tracking release: pins tollbooth-dpyc 0.87.1. Recorded after the fact —
the version shipped without a CHANGELOG entry at the time.
...
## [0.11.1] — 2026-07-09
- chore: track tollbooth-dpyc **0.62.1** — security-hardening batch. Adds an invoice-owner check on credit settlement (a settled invoice only credits the npub that created it), GCM-authenticated credential vault, and an encrypted self-provisioning ledger (of particular benefit to Authorities, which self-provision their own Neon). No plaintext audit. Pin bump + uv.lock regen; no wire-API change to this server.

## [0.11.0] — 2026-06-29
- chore: track tollbooth-dpyc **0.57.0** — the SDK unifies the Secure Courier possession token under one name `dpop_token` (retiring `proof`/`proof_token`/`poison`). The Authority's own tools take no caller `proof` param (proof handling lives in the wheel's `authority/tools.py`, renamed there), so this is a pin bump + uv.lock regen with no wire-API change to this server. Also picks up the wheel's free `patron_auth` probe on `service_status` and the proof-vs-credential flow cross-references.

## [0.10.2] — 2026-06-22

### Changed — consume SDK 0.52.0 (vault_source/purchase_mode decoupling)

- **chore: track tollbooth-dpyc through 0.52.0.** Picks up the vault_source/purchase_mode decoupling: Authorities now explicitly use `vault_source="env"` (self-provision Neon from env) and `purchase_mode="auto"` (derive direct/certified from registry chain). No wire-API changes. The Authority's server.py comments clarify its position in the chain (canonical Authority under Prime resolves to direct).
- Previously tracked: **0.49.0 — REQUIRED for the operator bootstrap NIP-33 switchover.** 0.49.0 publishes each operator's bootstrap config as a NIP-33 parameterized-replaceable event (kind 30078) instead of a kind-4 DM. This is a cold switchover with no fallback; after deploy, re-run `get_operator_config`/`register_operator` per operator to republish. (Also carried: deferred-adoption courtship flow, `repair_operator_schema`, the 0.45.3 refund-on-raise fix, and the 0.47.0 Authority-out-of-credits dunning.)
